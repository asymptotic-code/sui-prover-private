// Every `spec` parameter, in the #[mode(spec), ext(spec(...))] syntax.
// spec_params.legacy.move is the same file in the deprecated #[spec(...)] syntax.
...
#[mode(spec), ext(spec(prove, timeout = 60))]
...
#[mode(spec), ext(spec(prove, target = double, ignore_abort))]
...
#[mode(spec), ext(spec(prove, target = quarter, no_opaque, run_on = b"local"))]
...
#[mode(spec), ext(spec(prove, interpreted = square))]
...
#[mode(spec), ext(spec(prove, skip = b"not ready"))]
