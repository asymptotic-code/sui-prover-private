...
#[mode(spec), ext(spec(target = sui::tx_context::fresh_object_address))]
...
#[mode(spec), ext(spec(target = sui::tx_context::derive_id))]
fun derive_id_spec(tx_hash: vector<u8>, ids_created: u64): address {
    derive_id(tx_hash, ids_created)
}

#[mode(spec), ext(spec(target = sui::tx_context::fresh_id))]
fun fresh_id_spec(): address {
    fresh_id()
}

#[mode(spec), ext(spec(target = sui::tx_context::native_ids_created))]
fun native_ids_created_spec(): u64 {
    native_ids_created()
}

#[mode(spec), ext(spec(target = sui::tx_context::native_gas_budget))]
fun native_gas_budget_spec(): u64 {
    native_gas_budget()
}

#[mode(spec), ext(spec(target = sui::tx_context::last_created_id))]
fun last_created_id_spec(): address {
    last_created_id()
}

#[mode(spec), ext(spec(target = sui::tx_context::native_sponsor))]
