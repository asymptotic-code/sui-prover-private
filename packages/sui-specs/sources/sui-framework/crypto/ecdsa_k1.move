...
#[mode(spec), ext(spec(target = sui::ecdsa_k1::secp256k1_ecrecover))]
...
#[mode(spec), ext(spec(target = sui::ecdsa_k1::decompress_pubkey))]
public fun decompress_pubkey_spec(pubkey: &vector<u8>): vector<u8> {
    decompress_pubkey(pubkey)
}

#[mode(spec), ext(spec(target = sui::ecdsa_k1::secp256k1_verify))]
...
#[mode(spec), ext(spec(target = sui::ecdsa_k1::secp256k1_sign))]
...
#[mode(spec), ext(spec(target = sui::ecdsa_k1::secp256k1_keypair_from_seed))]
